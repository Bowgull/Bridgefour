...
export const projects: Project[] = [
  {
    ...
    problem: {
      ...
      body:
        "Aggregator emails, LinkedIn alerts, direct pitches, newsletter roll-ups. A job seeker burns an evening sorting the pile, scoring fit against their own profile, tracking who is worth a real application and who is noise. Most tools that claim to help add more noise. I watched people around me lose weekends to this and decided the pattern deserved a rebuild, so I built the radar.",
    },
    approach: {
      headline: "Ten sources. One ranked inbox. One click to apply.",
      body:
        "Sygnalist pulls from ten sources in one fetch. Remotive, RemoteOK, Jooble (which back-feeds Indeed, Monster, Glassdoor), Adzuna US, Adzuna Canada, USAJobs, plus a gated RapidAPI fallback for JSearch, LinkedIn last-7-days, and an ATS feed when the primary sources come back thin. A Gmail parser catches the rest: LinkedIn alerts, Greenhouse, Lever, Workday, Ashby, Breezy, Jobvite, iCIMS, SmartRecruiters. Each source has its own adapter with a request builder, response parser, and company-name normaliser. Behind all of it, one rule: no scheduled jobs, no hidden triggers, every action manual and visible. A user triggers a fetch. Twenty-five OpenAI enrichment requests fan out in parallel. A rule-based scorer applies hard disqualifiers and soft weights. Every job gets tiered S through X. Multi-profile from day one, one workbook as system of record, each user with their own portal mirrored from it.",
    },
    product: {
    ...
    tools: {
      ...
      body:
        "Sygnalist started as a Google Apps Script prototype so the v1 could live inside tools users already had. No login. No new tab. Once the idea held, I rebuilt it as a real fullstack app: Next.js 16 on Vercel for the portal, Supabase with SSR for auth and multi-profile data, Tailwind for the UI, Vercel cron for periodic fetches and stale-job cleanup. Ten job-source integrations behind a unified adapter dispatch. Claude and OpenAI both in the enrichment layer, Claude for why-fit narratives, OpenAI for structured classification. Twenty-five enrichments fan out in parallel to cut latency by ten times. Every fetch, promote, and error writes to a structured audit log. Non-destructive by design. Soft-lock a profile and it stops accepting writes, but every past action stays on the record.",
      list: [
        "Next.js 16",
    ...
    proof: {
      ...
      body:
        "Sygnalist is not for me. I already have a job. I built it because people around me were losing weekends to a broken system and nobody with the skills to fix it was bothering. Proved the concept on Google Apps Script first. Rebuilt on Next.js and Supabase once the idea had earned the rebuild. An 1,860-line BLUEPRINT governs every decision, from the voice rules (plain language, short sentences, no HR sludge) to the core law: manual, explicit, visible. Most AI tooling takes control away from the user. Sygnalist amplifies signal instead, and shows its reasoning so the person stays in charge. That is a design decision, not a feature.",
      stat: "2 rebuilds",
